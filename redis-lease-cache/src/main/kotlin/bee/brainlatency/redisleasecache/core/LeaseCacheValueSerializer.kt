...
/**
 * The value-serialization strategy behind [LeaseCacheEntryCodec]: how a cached value of
 * type [V] becomes payload bytes and back, decoupling the core from any concrete
 * serializer (Spring's `RedisSerializer`, JSON, ...). Nulls never reach it -- the codec
 * frames a negatively cached null as a bare tag with no payload -- so [serialize] deals
 * only in real, non-null values, which is why [V] is bounded `Any`.
 *
 ...
 */
interface LeaseCacheValueSerializer<V : Any> {
