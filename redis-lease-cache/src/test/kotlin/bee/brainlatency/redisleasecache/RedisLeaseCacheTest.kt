...
class RedisLeaseCacheTest : StringSpec({
    ...
    val cache = SpringRedisLeaseTokenCache("test-lease", LeaseTokenCache(store, codec, Duration.ofSeconds(5), Duration.ofSeconds(5)))
    ...
    "a cached value expires after valueTtl so the next call reloads" {
        val shortValueCache = SpringRedisLeaseTokenCache("short-value", LeaseTokenCache(store, codec, Duration.ofSeconds(5), Duration.ofMillis(300)))
    ...
    "a waiter gives up after waitTimeout while the lease is still held" {
        val impatientCache = SpringRedisLeaseTokenCache(
            "test-lease",
            LeaseTokenCache(
                store, codec,
